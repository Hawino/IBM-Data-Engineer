...
def extract(url, table_attribs):
    ...
    for row in rows:
        if row.find('td') is not None:
            ...
            df = pd.concat([df.dropna(), df1.dropna()], ignore_index=True, sort=False)
    return df
...
def transform(df):
   ...
   # Assuming the column names are 'Currency' and 'Rate'
...
#Query & Print 'Largest_banks'
query_statement = f"SELECT * FROM {table_name}"
run_query(query_statement, sql_connection)
#Query & Print the average market capitalization of all the banks in Billion USD.
query_statement = f"SELECT AVG(MC_GBP_Billion) FROM {table_name}"
run_query(query_statement, sql_connection)
#Query & Print top 5 banks
